...
def train(args):
    ...
    while maacktr.n_episodes < MAX_EPISODES:
        ...
        if maacktr.episode_done and ((maacktr.n_episodes + 1) % EVAL_INTERVAL == 0):
            ...
            print("Episode %d, Average Reward %.2f" % (maacktr.n_episodes + 1, rewards_std))
            eval_rewards.append(rewards_mu)

            eval_rewards_std.append(rewards_std)

            # save the model
            maacktr.save(dirs['models'], maacktr.n_episodes + 1)
    
    np.save(os.path.join(output_dir, 'eval_rewards.npy'),np.asarray(eval_rewards, dtype=float))

    np.save(os.path.join(output_dir, 'eval_rewards_std.npy'),np.asarray(eval_rewards_std, dtype=float))

    # save the model
    maacktr.save(dirs['models'], MAX_EPISODES + 2)
    plt.figure()
    plt.plot(eval_rewards)
    plt.plot(eval_rewards_std)
    plt.xlabel("Episode")
    plt.ylabel("Average Reward")
    plt.legend(["MAACKTR", "MAACKTR Std"])

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "learning_curve.png"), dpi=400, bbox_inches="tight")
...
def evaluate(args):
    ...
    rewards, speeds_positions, steps, avg_speeds = maacktr.evaluation(env, video_dir, len(seeds), is_train=False)

    print(rewards)
    print(len(rewards))

    log_dir = os.path.join(args.model_dir, 'eval_logs')
    csv_path = os.path.join(log_dir, 'eval_log.csv')
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['episode', 'reward', 'steps', 'avg_speed'])
        for i in range(len(rewards)):
            w.writerow([i, float(rewards[i]), int(steps[i]), float(avg_speeds[i])])
